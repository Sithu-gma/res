              ...
              <form action="{{url("/dish/$dish->id")}}" method="post" enctype="multipart/form-data">
                 ...
                 @method('PUT')
                   <div class="form-group">
                    ...
                    <input type="text" id="name" class="form-control" name="name" value="{{$dish->name}}">
                   </div>
                   ...
                   <div class="form-group">
                    ...
                    <select name="category_id" id="" class="form-control">
                        <option value="">Choose Category</option>
                          @foreach($cat as $cats)
                            <option value="{{$cats->id}}" {{$cats->id==$dish->category_id ? "selected":""}}>{{$cats->name}}</option>
                          @endforeach
                    </select>                  
                   </div>
                   ...
                   <img src="{{url("/images/$dish->image")}}" alt="" width="100px" height="100px">
                   <input type="file" id="cat" name="dish_image" >
                   </div>
